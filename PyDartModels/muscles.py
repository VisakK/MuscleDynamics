...
class Thelen2003():
    ...
    def force_l(self, lm, gammal=None):
        ...
        if gammal is None: gammal = 0.45#self.P['gammal']
    ...
    def velo_fm(self, fm, a, fl, lmopt=None, vmmax=None, fmlen=None, af=None):
        ...
        if lmopt is None: lmopt = 0.055#self.P['lmopt']
        if vmmax is None: vmmax = -0.45#self.P['vmmax']
        if fmlen is None: fmlen = 1.4#self.P['fmlen']
        if af is None: af = 0.25#self.P['af']
    ...
    def vm_eq(self, t, lm, lm0, lmt0, lmopt, ltslack, alpha0, vmmax, fm0,a):
        """Equation for muscle velocity."""

        if lm < 0.1*lmopt:
            lm = 0.1*lmopt
        #lt0 = lmt0 - lm0*np.cos(alpha0)        
        a = a#self.activation(t)
    ...
    def lm_sol(self, fun=None, t0=0, t1=3, lm0=None, lmt0=None, ltslack=None, lmopt=None,
               alpha0=None, vmmax=None, fm0=None,  show=True, axs=None,a=0.):
        ...
        if fun is None:
            fun = self.vm_eq
        f = ode(fun).set_integrator('dopri5', nsteps=100, max_step=0.0005, atol=1e-8)  
        f.set_initial_value(lm0, t0).set_f_params(lm0, lmt0, lmopt, ltslack, alpha0, vmmax, fm0,a)
